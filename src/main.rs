...
fn log_incident(msg: &str) {
        ...
        .open(INCIDENT_LOG_PATH)
    {
        ...
        Err(e) => {
            eprintln!("[LOG ERROR] Failed to write incident log: {}", e);
        }
    }
}
...
/* ============================================================
   PAID: BUILT-IN + LOCAL CUSTOM + SYSTEM CUSTOM RULES
============================================================ */
...
fn load_all_rules() -> Vec<Rule> {
    ...
    log_incident("[RULES] Loading Aegira Paid rules...");

    // 1. Project built-in rules
    if let Some(project_builtin) = project_builtin_rules_dir() {
        if project_builtin.exists() {
            let builtin = load_rules_from_directory(&project_builtin);
    ...
    // 2. System built-in rules
    let system_builtin = Path::new(SYSTEM_BUILTIN_RULES_DIR);

    if system_builtin.exists() {
        let builtin = load_rules_from_directory(system_builtin);
    ...
    if let Some(project_custom) = project_custom_rules_dir() {
        if project_custom.exists() {
            let custom = load_rules_from_directory(&project_custom);
    ...
    if rules.is_empty() {
        log_incident(
            "[RULES] No external rules loaded. Using hardcoded fallback."
        );

        let fallback = get_hardcoded_default_rules();

        for rule in fallback {
            seen_ids.insert(rule.id.to_lowercase());
            rules.push(rule);
        }
    }
...
fn calculate_match_score(
    ...
    let error_score: i32 =
        60 + (error_matches.saturating_sub(1) as i32 * 10);

    let context_score: i32 =
        context_matches as i32 * 10;

    let priority_score: i32 =
...
fn process_incident(
    ...
    match recover_with_rule(rule) {
        Ok(()) => {
            log_incident(&format!(
                "[RESOLVED] Incident automatically recovered in {:?}",
                start.elapsed()
            ));
        }
...
fn main() {
    ...
    log_incident(
        "[INFO] Aegira Paid Recovery Engine Started"
    );
